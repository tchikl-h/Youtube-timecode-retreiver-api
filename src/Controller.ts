...
export default class Controller {
    ...
    getChunkVideoListFromChannelId(channelId: string) {
        return new Promise<any>((resolve, reject) => {
            ...
            request.get(url, { qs: payload, headers: { Authorization: 'Bearer '+this.access_token} }, (error, response, body) => {
    ...
    getCaptionFromCaptionId(captionId: string) : Promise<string> {
        return new Promise<any>((resolve, reject) => {
            ...
            request({uri: url, method: 'GET', headers: {Authorization: 'Bearer '+this.access_token}}, (error, response, body) => {
                resolve(body);
            });
        });
    }
